...
password, the proxy URL that the guest fetches its sets through, and
the architecture. The method passes them to F<install.exp> in that
order, after the host and the port.
