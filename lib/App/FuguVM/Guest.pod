...
The tool generates a root password only in the C<expect> mode. In the
C<autoinstall> mode the response file owns every credential, and in
the C<import> mode the publisher of the image owns them. Outside the
C<expect> mode, C<up> therefore behaves by configuration:

=over 4

=item * With C<ssh_pubkey> and C<root_password_file>: read the
password from the first line of the file, store it in the state, and
install the key over a password login, exactly as after an expect
install. The tool warns when the file mode lets the group or another
user read the file.

=item * With no C<ssh_pubkey>: wait for SSH with the key of the
operator. The image must trust that key already.

=item * With C<ssh_pubkey> and no C<root_password_file>: the
configuration loader refuses with exit code 3.

=back

A published image and a consuming host share one credential: the
response file of the build sets the root password, the stack keeps
the password in a file, and each consuming host names that file with
C<root_password_file>. The metadata of an imported cache entry holds
no root password, because the tool must not invent a credential for
an image that it did not install.
...
=head2 accel, bind_address, connect_address, ssh_port, console_port

The facts of the guest that C<status> reports and that the guest
connections use.

C<accel> returns C<kvm>, C<hvf> or C<tcg>. While the guest runs, the
answer is the recorded value: a guest that started under
C<--emulate> runs TCG whatever the host can do now. In every other
case the answer is the accelerator that the tool selects now.

C<bind_address> returns the configured host address of the forwarded
ports. C<connect_address> returns the address that the tool connects
to: the bind address, or C<127.0.0.1> when the bind address is
C<0.0.0.0>, because C<0.0.0.0> is not a destination.

C<ssh_port> and C<console_port> return the recorded port while the
guest runs, and fall back to the configured number. They return
C<undef> for C<auto> in every other case: the record describes one
run, so a stopped guest has no port. Thus a record that a crash left
behind cannot read as a live port.

=head1 ADDRESSES AND PORTS

QEMU binds the forwarded SSH port and the serial console to
C<bind_address>. The default is C<127.0.0.1>, so no other machine
reaches a guest. C<bind_address 0.0.0.0> restores the wider reach.
Every connection that the tool makes to its own guest uses
C<connect_address>.

C<up> and C<start> resolve both ports one time for each invocation,
before the first spawn, and record them with the accelerator in
L<App::FuguVM::State>. A directive of C<auto> takes the first free
port of a fixed range: 100 ports from the default port of the
directive. The probe binds a listening socket on the bind address
and the candidate port. It skips every port that the record of a
sibling guest of the project holds. It also skips every fixed port
that a VM declaration of the project holds, stopped or not. An
exhausted range stops the run with exit code 1.

An exclusive C<flock(2)> on F<ports.lock> in the cache directory
covers the probe and the record of both ports, with a deadline of 30
seconds. Every project that shares the cache directory shares the
lock, so the probes serialize while every holder is inside its
deadline. The record exclusion covers the guests of one project
only. One window stays open: a
foreign process, a guest of an other project included, can take a
probed port before QEMU binds it. QEMU then fails to start, and the
tool reports that failure with the QEMU log. On the deadline the
