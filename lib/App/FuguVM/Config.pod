...
Return the merged configuration of one VM as a hash reference. Return
C<undef> when no file declares it, or when a value does not validate.
In the second case, C<error> reports the reason.
...
=head2 cache_dir, state_dir, default_vm, ssh_pubkey
